...
### 2. デザインアップデート 🎨
- [x] ~~アイテムアイコン機能の実装~~ ✅ **完了**
  - [x] アイテムに絵文字アイコンを登録できる機能
  - [x] アイコン選択UI（絵文字ピッカー）
  - [x] アイコン表示の実装
  - [x] テストの更新（全49テスト成功）
  - 実装詳細:
    - `emoji_picker_flutter` パッケージを導入
    - Itemモデルにiconフィールド（String?）を追加
    - ItemFormScreenにアイコン選択機能を統合
    - ItemCardにアイコン表示を追加
    - JSON シリアライゼーション対応

- [x] ~~カラーテーマの刷新~~ **完了**
  - [x] アプリアイコンのカラーコード（茶色・ベージュ・オレンジ系）を基調としたデザイン
  - [x] 温かみのあるカラーパレットの定義
...
**技術要件**:
- ~~emoji_picker_flutter（絵文字選択用）~~ ✅ **導入済み**
- カスタムテーマの拡張
- アニメーションライブラリ（flutter_animate など）

**実装の流れ**:
1. カラーパレットの定義と適用
2. ~~Itemモデルにiconフィールドを追加~~ ✅ **完了**
3. ~~アイコン選択UIの実装~~ ✅ **完了**
4. アイテムカードのデザイン刷新
5. ボタンとタイポグラフィの改善
6. アニメーション効果の追加
...
### 最近の変更
- 2026-01-06: **アイテムアイコン機能の実装完了** ✅
  - `emoji_picker_flutter` パッケージの導入
  - Itemモデルにiconフィールド（String?）を追加
  - `EmojiPickerWidget` と `showEmojiPickerBottomSheet` の実装
  - ItemFormScreenにアイコン選択機能を統合
  - ItemCardにアイコン表示を追加
  - ItemServiceの`createItem`と`updateItem`にiconパラメータを追加
  - 全テストを更新（全49テスト成功）
  - JSON シリアライゼーション対応
