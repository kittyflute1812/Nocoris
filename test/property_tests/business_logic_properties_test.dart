    ...
    group('プロパティ 6: インクリメント操作の正確性', () {
      test('Property 6: インクリメント操作の正確性 - **検証対象: 要件 3.1**', () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          // テスト用のアイテムをサービスに追加
          await itemService.createItem(testItem.name, testItem.count,
              icon: testItem.icon);
    ...
    group('プロパティ 7: デクリメント操作の正確性', () {
      test('Property 7: デクリメント操作の正確性 - **検証対象: 要件 3.2**', () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          expect(success, isTrue,
              reason:
                  'Decrement operation should succeed on iteration ${i + 1}');

          // プロパティ検証: アイテムの数量が正確に1減少している
          final updatedItem = itemService.getItemById(createdItem.id);
          expect(updatedItem, isNotNull,
              reason:
                  'Item should exist after decrement on iteration ${i + 1}');
          expect(updatedItem!.count, equals(initialCount - 1),
    ...
    group('プロパティ 10: アイテム削除の完全性', () {
      test('Property 10: アイテム削除の完全性 - **検証対象: 要件 5.2**', () async {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          expect(success, isTrue,
              reason: 'Delete operation should succeed on iteration ${i + 1}');
          ...
          expect(itemExists, isFalse,
              reason: 'Item should be deleted on iteration ${i + 1}');
          expect(lengthDecreased, isTrue,
              reason: 'List length should decrease by 1 on iteration ${i + 1}');
        }
      ...
      test('Property 10: 複数アイテム中の特定アイテム削除', () async {
        for (int i = 0; i < 50; i++) {
          ...
          for (final item in items) {
            await itemService.createItem(item.name, item.count,
                icon: item.icon);
          ...
          final initialLength = itemService.items.length;
          final targetIndex =
              PropertyTestHelpers.generateValidCount() % createdItems.length;
          ...
          expect(targetExists, isFalse,
              reason: 'Target item should be deleted');
          expect(lengthDecreased, isTrue,
              reason: 'List length should decrease by 1');
